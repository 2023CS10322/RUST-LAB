        ...
        *status_msg = "Cache cleared".to_string();
        ...
        if parts.len() == 2 {
            ...
            if let Some((row, col)) = cell_name_to_coords(cell_ref) { // [1, 3]
                if row < 0 || row >= sheet.total_rows || col < 0 || col >= sheet.total_cols {
                    *status_msg = format!("Cell {} out of bounds", cell_ref.to_uppercase()).to_string();
                    ...
                    #[cfg(feature = "cell_history")] // [4]
                    {
                        ...
                        match sheet.get_cell_history(row, col) {
                            ...
                            _ => { // Cell exists but has no history, or cell doesn't exist yet
                                *status_msg = format!("No recorded history for {}", cell_ref.to_uppercase()).to_string();
...
use egui_extras::{Size, StripBuilder, TableBuilder, Column}; // Added Column
...
use egui_plot::{Bar, BarChart, Legend, Plot, PlotPoints, Line,Points};
use egui::Vec2b; // For axis configuration
use egui::ComboBox; // For the dropdown
// Add linreg import
...
impl MyApp {
    ...
    fn get_cell_raw_content(&self, row: i32, col: i32) -> String {
         ...
         self.spreadsheet.get_formula(row, col)
             // Fallback to showing the numeric value if no formula exists
             .unwrap_or_else(|| self.spreadsheet.get_cell_value(row, col).to_string()) // get_cell_value returns i32 [1]
    }
        ...
        match (coord1_opt, coord2_opt) {
            (Some(coord1_tuple), Some(coord2_tuple)) => {
                // Check bounds using fields from Spreadsheet [1]
                 if coord1_tuple.0 < 0 || coord1_tuple.0 >= self.spreadsheet.total_rows || coord1_tuple.1 < 0 || coord1_tuple.1 >= self.spreadsheet.total_cols ||
                    coord2_tuple.0 < 0 || coord2_tuple.0 >= self.spreadsheet.total_rows || coord2_tuple.1 < 0 || coord2_tuple.1 >= self.spreadsheet.total_cols {
        ...
        match self.parse_range(&self.chart_config_range_data) {
            Ok(((r1, c1), (r2, c2))) => {
                ...
                if self.chart_config_x_labels.len() != num_rows {
                    self.chart_config_x_labels = (0..num_rows).map(|i| format!("Row {}", r1 + 1 + i as i32)).collect();
                }
                // Resize/populate names (use default column letters)
                if self.chart_config_line_names.len() != num_cols {
                     self.chart_config_line_names = (0..num_cols).map(|i| col_to_letters(c1 + i as i32)).collect();
                }
            }
            Err(e) => {
        ...
        match self.chart_config_type {
            ...
            ChartType::Bar => {
                ...
                if self.chart_config_parsed_dims.is_none() {
                    ...
                    if self.chart_config_parsed_dims.is_none() { return; } // Error already set
                }

                let range_result = self.parse_range(&self.chart_config_range_data);
                if let Err(e) = range_result { self.chart_error_message = e; return; }
                ...
                for i in 0..num_cols {
                    ...
                    let series_name = self.chart_config_line_names.get(i)
                                     .cloned()
                    ...
                    for j in 0..num_rows {
                        ...
                        let value = self.spreadsheet.get_cell_value(current_row, current_col) as f64;
                        if self.spreadsheet.get_cell_status(current_row, current_col) == CellStatus::Error {
                            self.chart_error_message = format!("Error in cell: {}", coords_to_cell_name(current_row, current_col));
                            return;
                        }
            ...
            ChartType::Line => {
                ...
                if self.chart_config_parsed_dims.is_none() {
                    ...
                    if self.chart_config_parsed_dims.is_none() { return; } // Error msg already set
                }
                ...
                for i in 0..num_cols { // Iterate Columns
                    let current_col = c1 + i as i32;
                    let line_name = self.chart_config_line_names.get(i)
                                     .cloned()
                                     .unwrap_or_else(|| col_to_letters(current_col));

                    let mut points: Vec<[f64; 2]> = Vec::with_capacity(num_rows);

                    for j in 0..num_rows { // Iterate Rows
                        let current_row = r1 + j as i32;
                        ...
                        if self.spreadsheet.get_cell_status(current_row, current_col) == CellStatus::Error {
                            self.chart_error_message = format!("Error in value cell: {}", coords_to_cell_name(current_row, current_col));
                            return;
                        }
            ...
            ChartType::Scatter => {
                ...
                if let Err(e) = x_range_result { self.chart_error_message = e; return; }
                let x_range = x_range_result.unwrap();

                let y_range_result = self.parse_range(&self.chart_config_range_y_values);
                if let Err(e) = y_range_result { self.chart_error_message = e; return; }
                let y_range = y_range_result.unwrap();

                // 2. Validation (as before)
                let x_len = (x_range.1.0 - x_range.0.0 + 1) * (x_range.1.1 - x_range.0.1 + 1);
                if x_len == 0 { /* error */ return; }
                let y_len = (y_range.1.0 - y_range.0.0 + 1) * (y_range.1.1 - y_range.0.1 + 1);
                if x_len != y_len { /* error */ return; }
                ...
                for i in 0..x_len {
                    let (x_r, x_c) = if x_is_col { (x_range.0.0 + i, x_range.0.1) } else { (x_range.0.0, x_range.0.1 + i) };
                    let (y_r, y_c) = if y_is_col { (y_range.0.0 + i, y_range.0.1) } else { (y_range.0.0, y_range.0.1 + i) };

                    let x_value = self.spreadsheet.get_cell_value(x_r, x_c) as f64;
                    if self.spreadsheet.get_cell_status(x_r, x_c) == CellStatus::Error { /* error */ return; }
                    let y_value = self.spreadsheet.get_cell_value(y_r, y_c) as f64;
                     if self.spreadsheet.get_cell_status(y_r, y_c) == CellStatus::Error { /* error */ return; }
                ...
                match linear_regression::<f64, f64, f64>(&xs, &ys) {
                    Ok((slope, intercept)) => {
                        ...
                        if let (Some(min_x), Some(max_x)) = xs.iter().fold((None, None), |(min_acc, max_acc), &x| {
                            let new_min = min_acc.map_or(Some(x), |min_val| Some(x.min(min_val)));
                            let new_max = max_acc.map_or(Some(x), |max_val| Some(x.max(max_val)));
                            (new_min, new_max)
                        }) {
                            ...
                            trendline_data = Some(vec![[min_x, y_at_min_x], [max_x, y_at_max_x]]);
                        } else {
                            self.chart_error_message = "Could not determine X range for trendline.".to_string();
                        }
                    }
                    Err(err) => {
                        // Regression failed (e.g., insufficient data, vertical line)
                        // Optionally provide more specific message based on linreg::Error type
                        self.chart_error_message = format!("Could not calculate trendline: {:?}", err);
                    }
                }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("formula_panel").show(ctx, |ui| {
             ui.horizontal(|ui| {
                ...
                ui.label("Selected:");
                ui.add_sized([60.0, ui.available_height()], egui::Label::new(&selected_label));
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            TableBuilder::new(ui)
                ...
                // --- End FIX 1 ---
                .header(20.0, |mut header| { // Header row height
                    // --- Column Headers ---
                    header.col(|ui| { ui.strong(""); }); // Top-left corner empty
        ...
        if self.show_chart_config_window {
            ...
            egui::Window::new("Chart Configuration")
                ...
                .show(ctx, |ui| {
                    ...
                    let chart_type_changed = ui.horizontal(|ui| {
                        let mut changed = false;
                        ui.label("Chart Type:");
// --- Update Bar Text ---
                        changed |= ui.selectable_value(&mut self.chart_config_type, ChartType::Bar, "Grouped Bar").changed();
                        changed |= ui.selectable_value(&mut self.chart_config_type, ChartType::Line, "Line").changed();
                        // --- Add Scatter Option ---
                        changed |= ui.selectable_value(&mut self.chart_config_type, ChartType::Scatter, "Scatter").changed();
                        changed
                    }).inner;
                    if chart_type_changed && (self.chart_config_type == ChartType::Bar || self.chart_config_type == ChartType::Line) {
                        self.update_dynamic_chart_config_dims();
                    }
                    ui.separator();
                    ...
                    match self.chart_config_type {
                        ...
                        ChartType::Bar => {
                            ...
                            if ui.text_edit_singleline(&mut self.chart_config_range_data).changed() {
                                self.update_dynamic_chart_config_dims(); // Use shared helper
                            }
                            ...
                            if let Some((num_rows, num_cols)) = self.chart_config_parsed_dims {
                                ...
                                if self.chart_config_x_labels.len() == num_rows {
                                     egui::ScrollArea::vertical().id_source("bar_cat_scroll").max_height(100.0).show(ui, |ui|{
                                        for i in 0..num_rows { 
                                            ui.horizontal(|ui| {
                                                ui.label(format!("Category {}:", i + 1));
                                                ui.text_edit_singleline(&mut self.chart_config_x_labels[i]);
                                            });/* ... input for chart_config_x_labels[i] ... */ }
                                     });
                                }
                                ui.separator();
                                ui.label("Series Names (Columns):"); // Renamed from Line Names
                                if self.chart_config_line_names.len() == num_cols {
                                     egui::ScrollArea::vertical().id_source("bar_ser_scroll").max_height(100.0).show(ui, |ui|{
                                         for i in 0..num_cols { 
                                            ui.horizontal(|ui| {
                                                ui.label(format!("Col {}:", i + 1)); // Label based on index
                                                ui.text_edit_singleline(&mut self.chart_config_line_names[i]);
                                            });/* ... input for chart_config_line_names[i] ... */ }
                                     });
                                }
                            } else {
                        ...
                        ChartType::Line => {
                            ...
                            if let Some((num_rows, num_cols)) = self.chart_config_parsed_dims {
                                ...
                                if self.chart_config_x_labels.len() == num_rows {
                                    ...
                                    egui::ScrollArea::vertical().id_source("x_axis_label_scroll").max_height(100.0).show(ui, |ui| {
                                ...
                                if self.chart_config_line_names.len() == num_cols {
                                     egui::ScrollArea::vertical().id_source("line_name_scroll").max_height(100.0).show(ui, |ui| {
                                        for i in 0..num_cols {
                                            ui.horizontal(|ui| {
            ...
            if !is_open { self.show_chart_config_window = false; self.chart_error_message.clear(); }
        }
       ...
       if let Some(chart_data) = &self.chart_to_display {
            ...
            if !is_display_open { close_chart_display = true; }
       }
       if close_chart_display { self.chart_to_display = None; }
...
pub fn main() -> Result<(), eframe::Error> {
    ...
    let options = eframe::NativeOptions {
        ...
        // You can also set other properties here, e.g.:
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile_error!("Features 'cli_app' and 'gui_app' are mutually exclusive. Please enable only one.");
