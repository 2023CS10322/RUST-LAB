...
mod gui_app {
        ...
        fn generate_chart_data(&mut self) {
            ...
            match self.chart_config_type {
                ...
                ChartType::Scatter => {
                    ...
                    if x_len == 0 {
                        self.chart_error_message = "X-Values range is empty.".to_string();
                        return;
                    }
                    let y_len =
                    ...
                    if x_len != y_len {
                        self.chart_error_message =
                            "X and Y ranges must have the same number of cells.".to_string();
                        return;
                    }

                    // Expect 1D ranges (single column or single row)
                    let x_is_col = x_range.0 .1 == x_range.1 .1;
                    let y_is_col = y_range.0 .1 == y_range.1 .1;
                    // Basic check: both columns or both single rows (can be more flexible)
                    if !((x_is_col && y_is_col)
                        || (!x_is_col
                            && !y_is_col
                            && x_range.0 .0 == x_range.1 .0
                            && y_range.0 .0 == y_range.1 .0))
                    {
                        // self.chart_error_message = "Scatter plot ranges must be single columns or single rows.".to_string(); return;
                    }
                    ...
                    for i in 0..x_len {
                        ...
                        // Get X value
                        let x_value = self.spreadsheet.get_cell_value(x_r, x_c);
                        if self.spreadsheet.get_cell_status(x_r, x_c) == CellStatus::Error {
                            self.chart_error_message =
                                format!("Error in X-value cell: {}", coords_to_cell_name(x_r, x_c));
                            return;
                        }

                        // Get Y value
                        let y_value = self.spreadsheet.get_cell_value(y_r, y_c);
                        if self.spreadsheet.get_cell_status(y_r, y_c) == CellStatus::Error {
                            self.chart_error_message =
                                format!("Error in Y-value cell: {}", coords_to_cell_name(y_r, y_c));
                            return;
                        }

                        points.push([x_value as f64, y_value as f64]);
                    }
                    ...
                    self.chart_to_display = Some(ChartData::Scatter(ScatterChartData {
                        title: self.chart_config_title.clone(),
                        points,
                        // point_labels: vec![], // Initialize if added
                    }));
                }
            }
    ...
    impl eframe::App for MyApp {
        fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            // --- Central Panel (Spreadsheet Grid - Fallback Simple Grid) ---
            egui::CentralPanel::default().show(ctx, |ui| {
                // TODO: Implement virtual scrolling using egui_extras::TableBuilder later
                ui.label("Spreadsheet (Simple Grid - Consider TableBuilder for large sheets)");
                egui::ScrollArea::both()
                    .auto_shrink([false; 2])
                    .show(ui, |ui| {
                        egui::Grid::new("spreadsheet_grid")
                            .num_columns(self.spreadsheet.total_cols as usize + 1)
                            .striped(true)
                            .show(ui, |ui| {
                                // Column Headers
                                ui.label(""); // Top-left corner
                                for c in 0..self.spreadsheet.total_cols {
                                    ui.label(col_to_letters(c));
                                }
                                ui.end_row();

                                // Rows (Limited for performance in simple grid)
                                // Let's show a limited number, e.g., 100 rows max in this simple view
                                let max_rows_to_display = self.spreadsheet.total_rows.min(100);
                                for r in 0..max_rows_to_display {
                                    ui.label(format!("{}", r + 1)); // Row header
                                    for c in 0..self.spreadsheet.total_cols {
                                        let is_selected = self.selected_cell == Some((r, c));
                                        // get_cell_status exists [1], CellStatus enum exists [1]
                                        let cell_value_str = match self
                                            .spreadsheet
                                            .get_cell_status(r, c)
                                        {
                                            CellStatus::Error => "ERR".to_string(),
                                            _ => self.spreadsheet.get_cell_value(r, c).to_string(), // get_cell_value exists [1]
                                        };
                                        let response = ui.add_sized(
                                            [80.0, 18.0], // Fixed cell size
                                ...
                                if max_rows_to_display < self.spreadsheet.total_rows {
                                    ui.label("..."); // Indicate more rows exist
                                    ui.end_row();
                                }
                            });
                    });
                ...
                .show(ctx, |ui| {
                    ...
                    plot.show(ui, |plot_ui| {
                        match &chart_data_clone {
                            ...
                            // --- Add Scatter Plotting ---
                            ChartData::Scatter(scatter_data) => {
                                // Convert Vec<[f64; 2]> to PlotPoints
                                let plot_points = PlotPoints::from(scatter_data.points.clone());
                                // Create Points item
                                let points = Points::new(plot_points)
                                    .radius(3.0) // Set point size
                                    // .color(egui::Color32::BLUE) // Set color
                                    .name(&scatter_data.title); // Use chart title as series name for legend
                                // Add points to the plot [5]
